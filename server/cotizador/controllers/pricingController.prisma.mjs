/**
 * Pricing Controller - PRISMA VERSION
 ...
 */
export const updateMultipliers = async (req, res) => {
  try {
 ...
 */
export const getInternalView = async (req, res) => {
  try {
    // Verificar que sea admin
    if (req.user?.role !== 'ADMIN') {
      return res.status(403).json({
...
export const getPublicCatalog = async (req, res) => {
  try {
    ...
    if (tipo_prenda_codigo || disponibilidad) {
      ...
      if (disponibilidad && disponibilidad !== 'disponible') params.push(disponibilidad)
